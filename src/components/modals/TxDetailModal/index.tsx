        ...
        return transaction.txInfo?.view?.bodyView?.actionViews.map(i => {
            ...
            if (type === 'spend') {
                ...
                try {
                    ...
                    const amount =
                        //@ts-ignore
                        assetId.includes("nft") ? Number(i.actionView.value.outputView.value.note.value.valueView.value.amount.lo) : Number(i.actionView.value.outputView.value.note.value.valueView.value.amount.lo) / 10 ** 6

                    return {
                        text:
                            addresView.case === 'opaque'
                                ? `${amount} ${assetId} to ${getShortKey(
                                    address
                                )}`
                                : `${amount} ${assetId}`,
